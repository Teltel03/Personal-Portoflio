...
// MAS MAGANDA KUNG ILAN YUNG MAX ROWS PARA SAKTO
const MAX_NUMBER_OF_ATTEMPTS = 6;

const history = [];
let currentWord = '';
let timerId = null;
let secondsElapsed = 0; // Set the initial seconds elapsed
let wordGuessed = false; // Variable to track if the word is guessed  
...
const startTimer = () => {
  ...
  const updateTimer = () => {
    ...
    timerElement.textContent = `🕒 ${secondsElapsed} seconds`
  };
...
const resetGame = () => {
  ...
  rows.forEach(row => {
    ...
    columns.forEach(column => {
      column.textContent = '';
      column.setAttribute('data-status', 'empty');
    });
  });

 
  const keyboardKeys = document.querySelectorAll('#keyboard li[data-status]');
  keyboardKeys.forEach(key => {
    key.setAttribute('data-status', 'empty');
  });

  history.length = 0;
  currentWord = '';
  WORD_OF_THE_DAY = generateNewSecretWord();  // GENERATE NG SECRET WORD

  const remainingAttemptsElement = document.querySelector('#remainingAttempts');
  remainingAttemptsElement.textContent = MAX_NUMBER_OF_ATTEMPTS;
  

 
  wordGuessed = false; // Reset the wordGuessed variable
  ...
  if (history.length >= MAX_NUMBER_OF_ATTEMPTS) {
    showMessage(`Game over! The word was ${WORD_OF_THE_DAY}.`);
...
// Function to generate a new secret word
function generateNewSecretWord() {
  const newWordIndex = getRandomIndex(WORD_LIST.length);
  return WORD_LIST[newWordIndex];
}

let WORD_OF_THE_DAY = generateNewSecretWord(); // Generate initial secret word


const showMessage = (message) => {
  const toast = document.createElement('li');

  toast.textContent = `${message} Time: ${secondsElapsed} seconds`;
...
const checkGuess = (guess, word) => {
  ...
  const remainingAttemptsElement = document.querySelector('#remainingAttempts');
  remainingAttemptsElement.textContent = MAX_NUMBER_OF_ATTEMPTS - history.length - 1;
  
  history.push(currentWord);
  currentWord = '';

  if (wordLetters.join('') === guess) {z
    var playAgain = confirm("Congratulations! You found the word! Do you want to play again?");
    wordGuessed = true; // Set wordGuessed to true
...
function askToPlayAgain() {
      ...
      // If the player wants to play again, call the reset function
      if (playAgain) {
        reset();
      }
    }
...
const onKeyDown = (key) => {
  // 6 LETTERS LANG DAPAT OR KUNG ANO MAN NAKALAGAY
  ...
  if (key === ENTER_KEY) {
    ...
    if (currentWord.length === 5 && WORD_LIST.includes(currentWord)) {
      if (history.includes(currentWord)) {
        showMessage('You have already entered that word.');
      } else {
...
function toggleMode() {
  ...
  var toggleButton = document.toggleButton;
